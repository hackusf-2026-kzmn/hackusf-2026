...
const SEVERITY_OPTIONS = [
  { value: "Extreme", label: "Extreme", color: "#ff3b3b", desc: "Only the most severe emergencies" },
  { value: "Severe+", label: "Severe+", color: "#ff9f1a", desc: "Severe + Extreme alerts" },
  { value: "Moderate+", label: "Moderate+", color: "#ffd43b", desc: "Moderate + Severe + Extreme alerts" },
  { value: "Minor+", label: "Minor+", color: "#7dd3fc", desc: "All alerts including minor advisories" },
];
...
function EmailOptInModal({ onClose }: { onClose: () => void }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      await subscribe({ email: email.trim(), severity: selected, zip_code: zip.trim() });
      setStatus("success");
    } catch {
      setStatus("error");
    }
        ...
        {status === "success" ? (
          ...
          <div className="px-5 py-4 space-y-4">
            <p className="text-[10px] text-[#52665e] leading-relaxed">
              Get real-time crisis alerts delivered to your inbox. Choose a severity threshold for notifications.
            </p>
            ...
            <div>
              <label className="block text-[9px] text-[#6b7869] tracking-[1.5px] uppercase mb-1">
                Zip Code <span className="normal-case tracking-normal text-[#b8c4aa]">(required)</span>
              </label>
              <input
                type="text"
                value={zip}
            ...
            <button
              ...
              disabled={!email.trim() || !zip.trim() || status === "submitting"}
